...
abstract class RxObserver<T>() : Observer<T> {
    ...
    override fun onError(e: Throwable) {
        ...
        if (!NetUtil.isConnected(BaseApplication.instance())){
            // 网络问题
            doOnError(BaseApplication.instance().getString(R.string.no_net))
        }else if (e is ServerException){
            // 服务器
            doOnError(e.message?:"ServerException")
        }else if (e is TimeoutException){
            // 超时
            doOnError(BaseApplication.instance().getString(R.string.net_time_out))
        }else{
            doOnError(BaseApplication.instance().getString(R.string.net_error))
        }
    }
